...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 h-[76px] flex items-center justify-between gap-6">
        ...
        <button
          id="brand-logo"
          onClick={() =>
            handleNavClick(isAuthenticated ? 'dashboard' : 'landing')
          }
          className="flex items-center cursor-pointer group select-none shrink-0"
          aria-label="IntervuAI home"
        >
          <div className="flex items-center shrink-0 group-hover:scale-[1.03] transition-transform">
            <img
              src="/logo.jpeg"
              alt="IntervuAI"
        ...
        <nav className="hidden lg:flex items-center gap-0.5 flex-1 justify-center">
          {isAuthenticated ? (
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-950 transition-colors"
            >
              Home
            </button>
          )}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isAuthenticated ? (
            <div className="relative">
              ...
              >
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                  {user?.name
                    ? user.name.charAt(0).toUpperCase()
                    : 'U'}
                </div>

                <span className="hidden xl:block max-w-[120px] truncate text-sm font-semibold text-slate-800">
                  {user?.name || 'User'}
                </span>
                ...
                <div className="absolute right-0 mt-3 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10">
                  ...
                  <div className="p-2">
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-3"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 rounded-xl text-sm font-semibold text-rose-600 hover:bg-rose-50 flex items-center gap-3"
                    >
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 shadow-md shadow-indigo-500/20 hover:-translate-y-0.5 active:translate-y-0 transition-all"
              >
                Get Started Free
              </button>
            ...
            {isAuthenticated ? (
              <>
                ...
                {[
                  ...
                  <button
                    key={view}
                    onClick={() =>
                      handleNavClick(view as NavView)
                    }
              ...
              <>
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-3 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50"
                >
                  Log In
                </button>
                ...
                <button
                  ...
                  className="w-full text-center px-4 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600"
                >
                  Get Started Free
                </button>
